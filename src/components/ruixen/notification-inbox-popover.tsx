...
export function NotificationInboxPopover({
  ...
  isLoading = false,
}: NotificationInboxPopoverProps & { isLoading?: boolean }) {
    ...
    <Popover>
      <PopoverTrigger asChild>
        ...
        >
          ...
          {unreadCount > 0 && (
            <Badge className="absolute -top-2 right-full min-w-5 rounded-full translate-x-1/2  bg-destructive text-white hover:bg-destructive">
      ...
      <PopoverContent
        className={`${popoverWidth} p-0 shadow-xl border-muted/20`}
        align="end"
        alignOffset={16}
        sideOffset={8}
      >
