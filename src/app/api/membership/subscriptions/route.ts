...
export async function POST(request: Request) {
  try {
    ...
    let apiUrl = `${process.env.NEXT_PUBLIC_API_URL}/api/memberships/subscriptions`;

    console.log(
      `[Subscriptions API Proxy] Forwarding to backend URL: ${apiUrl}`,
    );
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      console.log(`[Subscriptions API Proxy] Rebuilding FormData:`);
    ...
    if (response.status >= 400) {
      console.error(
        `[Subscriptions API] Error ${response.status}:`,
        response.data,
      );
