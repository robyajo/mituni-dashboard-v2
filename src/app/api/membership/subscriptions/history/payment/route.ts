...
export async function POST(request: Request) {
  try {
    ...
    let apiUrl = `${process.env.NEXT_PUBLIC_API_URL}/api/memberships/subscriptions/history-payment`;

    console.log(
      `[Subscriptions History Payment API Proxy] Forwarding to backend URL: ${apiUrl}`,
    );
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      console.log(
        `[Subscriptions History Payment API Proxy] Rebuilding FormData:`,
      );
    ...
    if (response.status >= 400) {
      console.error(
        `[Subscriptions History API] Error ${response.status}:`,
        response.data,
      );
