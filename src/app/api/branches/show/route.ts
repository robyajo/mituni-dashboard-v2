...
export async function POST(request: Request) {
  try {
    ...
    const apiUrl = `${process.env.NEXT_PUBLIC_API_URL}/api/branches/branches_byid`;
    console.log(`[Branch Show Proxy] Forwarding to backend URL: ${apiUrl}`);
    ...
    if (response.status >= 400) {
      console.error(
        `[Branch Show Proxy] Error ${response.status}:`,
        response.data,
      );
    ...
    console.error(
      `[Branch Show Proxy] Error processing request:`,
      error.message,
    );
